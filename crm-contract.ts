...
cli({
  ...
  args: [
    ...
    { name: 'bizStatus', type: 'int', default: 0, help: '合同状态：0、草稿 1、审批中 2、审核通过未归档 3、已归档 4、审批不通过 5、已撤销' },
    { name: 'crmProductIds', type: 'string', default: '', help: '产品IDS数组，多个用逗号分隔，如：[1,2]' },
    { name: 'timeType', type: 'int', default: 1, help: '时间类型：1、合同开始时间 2、合同结束时间' },
    { name: 'startTime', type: 'int', default: 0, help: '开始时间戳' },
    { name: 'endTime', type: 'int', default: 0, help: '结束时间戳' },
  ],
  ...
  func: async (page: IPage, kwargs: Record<string, unknown>) => {
    ...
    return records.map((r) => ({
      ...
      ourCorpFullName: `${r.ourCorpFullName}-${OUR_CORP_FULL_NAME_MAP[r.ourCorpFullName] ?? '未知'}`,
      orderNum: r.orderNum,
      uid: r.uid,
      corpName: r.corpName,
      ...
      bizStatus: `${r.bizStatus}-${BIZ_STATUS_MAP[r.bizStatus] ?? '未知'}`,
      allowOrderStatus: r.allowOrderStatus,
      contractOrderStatus: r.contractOrderStatus,
      orderStatus: `${r.orderStatus}-${ORDER_STATUS_MAP[r.orderStatus] ?? '未知'}`,
